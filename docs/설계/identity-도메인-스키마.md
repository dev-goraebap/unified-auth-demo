...
1. `gender`를 `char(1) M/F`로 둘지, PASS 원본이 주는 코드(내/외국인·성별 코드)를 더 반영할지.
2. `social_accounts`에 소셜 프로필(이메일·닉네임) 캐시 컬럼을 둘지(로그인엔 불필요, 표시용).
3. RFT 회전 시 `replaced_by` 체인까지 둘지, 단순히 `revoked_at`만 둘지(데모 단순화).
4. `verification` 만료 청소를 스케줄러 배치로 할지, 조회 시 조건으로만 걸러낼지.

→ 큰 결정은 아니라 구현하며 정해도 되지만, 선호가 있으면 반영한다.
